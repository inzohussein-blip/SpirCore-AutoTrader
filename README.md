...
### 🚨 الأمان التشغيلي (Operational Safety)
| الإضافة | الوظيفة |
|---------|---------|
| **فلتر الجلسة (kill-switch)** | يمنع الأتمتة خارج ساعات محددة (`InpUseSession` + ساعات البداية/النهاية؛ تلتفّ حول منتصف الليل). حماية وقت الأخبار/التقلّب. |
| **سقف التعرّض الكلي** | `InpMaxTotalLots` — يمنع الدخول إذا تجاوز مجموع اللوت المفتوح الحد (ليس عدد الصفقات فقط). |
| **إعادة اتصال MT5 (الجسر)** | watchdog كل 10 ثوانٍ يعيد الاتصال تلقائياً إن انقطع الرابط + إشعار. |

> نفس فلتر الجلسة وسقف اللوت موجودان في الجسر (`USE_SESSION_FILTER`, `MAX_TOTAL_LOTS`).
...
### 🤖 بوتات ومؤشرات الاختبار (`MT5/Bots/`, `MT5/Indicators/`)
بوتات MQL5 **مستقلة تماماً** (بلا include) ومبسّطة — بوت لكل استراتيجية (5) لسحبها مباشرةً على الشارت أو **Strategy Tester** لمقارنة سريعة، + مؤشرات احترافية تُرسم على الشارت (ZLSMA، Chandelier Exit، مغلّف NW). التفاصيل في **[MT5/Bots/README.md](MT5/Bots/README.md)**.
...
## 💼 التسويق كمنتج (SaaS) — `saas/`
أساس تقني لتحويل النظام إلى منتج: **مستخدمون + تراخيص مربوطة بحساب MT5 + هيكل اشتراكات (Stripe) + صفحة أداء عامة + نسخ تلقائي (Copy-Trading)**، قابل للنشر عبر Docker (بلا اعتماد على MetaTrader5). يتحقق الـ EA من الترخيص عبر `InpUseLicense`، ويدفع الجسر الأداء عبر `SAAS_URL`، وينشر إشاراته للمتابعين عبر `SAAS_PUBLISH` (بوت `SpirBot_Follower.mq5` ينسخها). التفاصيل في **[saas/README.md](saas/README.md)**.
> ⚠️ قيمته الحقيقية تبدأ بعد **إثبات أفضلية على Demo بسجل مُدقَّق**؛ ولا تَعِد بأرباح مضمونة (اعتبارات قانونية).
